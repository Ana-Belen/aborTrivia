...
class AnswerContainer extends React.Component {
  ...
  getOtherOptions(list, current) {
    ...
    //current = current list of options so far
    ...
    for (let i; success !== true; i++) {
      ...
      for (let user of current) {
        // 2a If I find the user in the list of buttons or is the right answer,
  ...
  render() {
    ...
    return (
      <div className="options">
        <OptionButton options={dipArray} rightOne={this.props.rightAnswer} />
      </div>
    );
  }
}
